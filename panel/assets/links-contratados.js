...
(function () {
  ...
  function renderDetailView(order) {
    ...
    var agentHtml = "";
    if (order.agent_name) {
      agentHtml = '<div style="display:flex;align-items:center;gap:12px;padding:14px 18px;background:linear-gradient(135deg,#f0f9ff,#e0f2fe);border-radius:12px;border:1px solid #bae6fd">' +
        ...
        '<div><p style="margin:0;font-size:14px;color:#0c4a6e;font-weight:600">' + escapeHtml(order.agent_name) + '</p>' +
        (order.agent_phone ? '<p style="margin:2px 0 0;font-size:13px;color:#0891b2">' + escapeHtml(order.agent_phone) + '</p>' : '') + '</div></div>';
    }
